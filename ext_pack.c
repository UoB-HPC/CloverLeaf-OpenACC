        ...
        int *bffr_ffst, int* offload)
{
    ...
    // These array modifications still need to be added on, 
    // plus the donor data location changes as in update_halo
...
//#pragma ivdep
        for (int j = 1; j <= depth; j++) 
        {
            ...
            left_snd_buffer[FTNREF1D(index,1)] =
...
//#pragma ivdep
        ...
        {
            int index = buffer_offset+j+(k+depth-1)*depth;
            field[FTNREF2D(x_min-j,k,x_max+4+x_inc,x_min-2,y_min-2)] =
...
//#pragma ivdep
        ...
        {
            int index = buffer_offset+j+(k+depth-1)*depth;
            right_snd_buffer[FTNREF1D(index,1)] =
...
//#pragma omp parallel for 
        for (int j = x_min-depth; j <= x_max+x_inc+depth; j++) 
        {
            ...
            top_snd_buffer[FTNREF1D(index,1)] =
...
//#pragma omp parallel for 
        ...
        {
            int index = buffer_offset+k+(j+depth-1)*depth;
            bottom_snd_buffer[FTNREF1D(index,1)] =
...
//#pragma omp parallel for
        ...
        {
            int index = buffer_offset+k+(j+depth-1)*depth;
            field[FTNREF2D(j,y_min-k,x_max+4+x_inc,x_min-2,y_min-2)] =
...
//#pragma omp parallel for 
        ...
        {
            int index = buffer_offset + k+(j+depth-1)*depth;
            field[FTNREF2D(j,y_max+y_inc+k,x_max+4+x_inc,x_min-2,y_min-2)] =
